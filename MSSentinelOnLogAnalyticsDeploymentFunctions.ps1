...
function New-ArcDeploymentScript {
    [CmdletBinding()]
...
function New-ArcOnboardFolder {
    [CmdletBinding()]
    Param($TenantId, $SubscriptionId, $ResourceGroupName, $Location, [PSCredential]$ApplicationCredential, $Template, $Path)
    ...
    New-ArcDeploymentScript @arcParams | Out-File -FilePath $Path
}